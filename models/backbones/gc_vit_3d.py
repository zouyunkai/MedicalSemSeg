...
class ReduceSize(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.conv(x)
        x = self.reduction(x).permute(0, 2, 3, 4, 1)
        x = self.norm2(x)
        return x
...
class WindowAttentionGlobal3D(nn.Module):
    ...
    def forward(self, x, q_global):
        ...
        kv = self.qkv(x).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        q_global = q_global.repeat(B_//B, 1, 1, 1, 1)
        q = q_global.reshape(B_, self.num_heads, N, C // self.num_heads)
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))
...
def window_reverse3D(windows, window_size, S, H, W):
...
class GCViTBlock(nn.Module):
    def __init__(self,
                 ...
                 drop_path=0.,
                 act_layer=nn.GELU,
                 attention=WindowAttentionGlobal3D,
                 norm_layer=nn.LayerNorm,
                 layer_scale=None,
                 ):
        super().__init__()
        ...
        self.num_windows = int((input_resolution[0] // window_size) * (input_resolution[1] // window_size) * (input_resolution[2] // window_size))
    ...
    def forward(self, x, q_global):
            ...
            x = window_reverse3D(attn_windows, self.window_size, S, H, W)
...
class GCViTLayer(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            GCViTBlock(dim=dim,
                       ...
                       qkv_bias=qkv_bias,
                       qk_scale=qk_scale,
                       attention=WindowAttention3D if (i % 2 == 0) else WindowAttentionGlobal3D,
                       drop=drop,
                       attn_drop=attn_drop,
        ...
        elif input_resolution[0] == 24:
            self.to_q_global = nn.Sequential(
                FeatExtract(dim, keep_dim=False),
                FeatExtract(dim, keep_dim=False),
            )

        elif input_resolution[0] == 12:
    ...
    def forward(self, x):
        q_global = self.to_q_global(x.contiguous().view(-1,
                                           self.dim,
                                           self.resolution[0],
                                           self.resolution[1],
                                           self.resolution[2]))
...
class GCViT(nn.Module):
    def __init__(self,
        ...
        num_features = [int(dim * 2 ** (i + 1)) for i in range(self.num_layers)]
